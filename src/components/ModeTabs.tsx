...
export function ModeTabs() {
  return (
    <div className="mx-auto mt-3 grid w-full max-w-sm grid-cols-4 gap-1.5 rounded-2xl border border-border bg-card p-1.5 shadow-soft md:hidden">
      {tabs.map(({ to, label, icon: Icon }) => (
        <Link
          key={to}
          to={to}
          activeOptions={{ exact: to === "/" }}
          className="flex flex-col items-center justify-center gap-1 rounded-xl px-1 py-2 text-[10px] font-semibold text-muted-foreground transition-colors hover:bg-secondary"
